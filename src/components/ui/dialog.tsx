    ...
    <DialogPortal data-slot='dialog-portal'>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          'data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-[9999] grid w-full max-w-[calc(100%-2rem)]',
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            className={cn(
              ...
              'absolute top-10.5 right-9 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:outline-hidden md:top-8.5 md:right-9 xl:top-8',
...
function DialogFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div data-slot='dialog-footer' className={cn('flex justify-center', className)} {...props} />
  );
}
...
function DialogTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn('text-mogazoa-20px-600 xl:text-mogazoa-24px-600', className)}
      {...props}
    />
  );
}
